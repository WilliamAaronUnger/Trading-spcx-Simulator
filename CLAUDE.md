...
- Positions support shorting: `pos.qty < 0` is a short with `avg` = average short price. Selling with no/negative position opens/increases a short (capped by `maxShortQty`: open short exposure ≤ 1× `totalOf`, no leverage); buying while short covers (clamped, never crosses into long in one order). Generic formulas like `(cur - avg) * qty` already handle the sign — but percent displays and chart up/down logic must flip for `qty < 0`. The result-exchange payload version (currently `SPCX3.…`) must be bumped whenever stats fields change.
