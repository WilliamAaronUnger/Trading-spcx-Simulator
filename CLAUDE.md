...
- Positions support shorting: `pos.qty < 0` is a short with `avg` = average short price. Selling with no/negative position opens/increases a short (capped by `maxShortQty`: open short exposure ≤ 1× `totalOf`, no leverage); buying while short covers (clamped, never crosses into long in one order). Generic formulas like `(cur - avg) * qty` already handle the sign — but percent displays and chart up/down logic must flip for `qty < 0`. The result-exchange payload version (`SPCX2.…`) must be bumped whenever stats fields change.
...
- Per-round trading stats live in `players[i].stats` (filled in `trade()`/`processTick()`) and feed `playerTitle()` and `statsLine()` on the handover/result screens, plus the detailed `buildAnalysis()` table in the result modal.
- In remote mode the duel comparison works without a server via `packResult()`/`unpackResult()`: a versioned base64 string (`SPCX1.…`) carrying result + stats that players exchange manually; `renderCompare()` then shows the same two-column result view as local mode. The payload embeds `gameCode` so results from a different game are rejected, and externally supplied strings (name, favorite stock) must go through `esc()` before touching `innerHTML`.
